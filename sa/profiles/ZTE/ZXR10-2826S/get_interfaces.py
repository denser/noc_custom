...
class Script(BaseScript):
    """
    ZTE.ZXR10-2826S.get_interfaces
    """

    name = "ZTE.ZXR10-2826S.get_interfaces"
    interface = IGetInterfaces

    rx_int = re.compile(r"^\s+PortId\s+:\s+(?P<name>\d+).*PortEnable\s+:\s+(?P<status>\S+)")
    rx_link = re.compile(r"^.*Link\s+:\s+(?P<link>.+?)\s*$")
    rx_descr = re.compile(r"^.*Description:\s+(?P<descr>.+?)\s*$")

    rx_ethernet = re.compile(
            r"^\s+PortId\s+:\s+(?P<name>\d+)\s+.+\n"
            r"^name\s+: .+\n"
            r"^Pvid\s+: (?P<pvid>\d+)\s*\n"
    ...
    def execute(self):
        ...
        for l in self.cli("show port").splitlines():
            # New interface
        ...
        for l in self.cli("show lacp").splitlines():
